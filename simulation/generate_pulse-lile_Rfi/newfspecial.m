function h = newfspecial(varargin)
...
%
%   Depending on TYPE which can be a string or a char vector, FSPECIAL may
...
%   Use IMSHARPEN in place of the 'unsharp' mask syntax.
%
...
switch nargin
        ...
        switch type
            ...
            case {'gaussian','log','average'}
                validateattributes(p2,{'double'},...
                    ...
                    mfilename,'HSIZE',2);
                if numel(p2) > 2
                    error(message('images:fspecial:wrongSizeHSize'))
                elseif numel(p2)==1
                    p2 = [p2 p2];
                end
        end
    ...
    case 3
        % FSPECIAL('gaussian',N,SIGMA) or FSPECIAL('gaussian',[],SIGMA)
        % FSPECIAL('log',N,SIGMA) or FSPECIAL('log',[],SIGMA)
